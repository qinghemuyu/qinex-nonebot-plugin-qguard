...
- AI 售后中台：AI Core 模型底座、LogDoctor 日志诊断、GroupWiki 软件知识库问答、SupportBot 知识问答入口。
...
/知识 范围
/知识 范围 全部
/知识 范围 分类 06_连点与压枪,07_投屏ScreenHub
/知识 技能
/知识 查看 K0001
/知识 有用 K0001
/知识 没用 K0001
...
每个群可以单独选择知识库回答范围。默认是全部分类；设置分类后，本群的 `/问`、`/知识 问`、SupportBot `/求助` 都只会在这些分类里检索和回答。
...
SupportBot 是知识库智能问答入口，只根据 GroupWiki 当前群生效的知识范围回答。它不创建工单，也不接管日志诊断；日志、Traceback 仍然使用 LogDoctor 的 `/诊断` 或 `/报错`。
...
SupportBot 的群内回复标记为聊天消息，默认不会被 QGuard 自动撤回。知识范围由 `/知识 范围` 控制，默认全部分类。
