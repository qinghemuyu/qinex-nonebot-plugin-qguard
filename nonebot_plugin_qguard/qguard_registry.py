...
def get_qguard_descriptor() -> PluginDescriptor:
    commands = (
        ...
        _cmd("/管 自动清理 执行", "立即扫描并生成待确认清单", QGuardRole.GROUP_ADMIN, danger_level=1),
        _cmd("/管 自动清理 待确认", "查看长期未活跃待确认成员", QGuardRole.GROUP_ADMIN),
        _cmd("/管 自动清理 确认", "主人确认踢出待清理成员", QGuardRole.SUPER_ADMIN, danger_level=3),
        _cmd("/管 自动清理 确认 排除 QQ", "确认清理时临时排除成员", QGuardRole.SUPER_ADMIN, danger_level=3),
        _cmd("/管 自动清理 保留 QQ 原因", "加入自动清理保留名单", QGuardRole.GROUP_OWNER, danger_level=1),
        _cmd("/管 自动清理 取消", "取消自动清理待确认记录", QGuardRole.GROUP_OWNER, danger_level=1),
        ...
        _cmd("/管 入群欢迎 开", "开启新人入群欢迎", QGuardRole.GROUP_ADMIN, danger_level=1),
        _cmd("/管 入群欢迎 关", "关闭新人入群欢迎", QGuardRole.GROUP_ADMIN, danger_level=1),
        _cmd("/管 入群欢迎 状态", "查看入群欢迎配置", QGuardRole.GROUP_ADMIN),
        _cmd("/管 入群欢迎 模板 文本", "设置入群欢迎模板", QGuardRole.GROUP_ADMIN, danger_level=1),
