...
async def _(bot: Bot, event: GroupMessageEvent) -> None:
    ...
    if action in {"添加", "加入"}:
        result = await service.add_blacklist(event.group_id, event.user_id, parsed.user_id, parsed.rest or None)
        await finish_reply(blacklist_matcher, bot, event, result.message)

    result = await service.remove_blacklist(event.group_id, event.user_id, parsed.user_id)
    await finish_reply(blacklist_matcher, bot, event, result.message)
