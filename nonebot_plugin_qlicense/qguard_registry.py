...
def get_qguard_descriptor() -> PluginDescriptor:
    return PluginDescriptor(
        ...
        description="S3/P4 板子自助登记、QQ 授权配额和历史授权同步。",
        commands=(
            _cmd("/激活 S3 MAC", "登记自己的 S3 板子 MAC", QGuardRole.MEMBER),
            _cmd("/激活 P4 MAC", "登记自己的 P4 上位机 MAC", QGuardRole.MEMBER),
            _cmd("/激活 状态", "查看自己的授权额度", QGuardRole.MEMBER),
            _cmd("/授权 查询 QQ", "查询 QQ 授权状态", QGuardRole.SUPER_ADMIN),
            _cmd("/授权 配额 QQ 数量 [S3|P4]", "设置 QQ 的 S3/P4 配额", QGuardRole.SUPER_ADMIN),
            _cmd("/授权 默认配额 数量 [S3|P4]", "设置全局默认 S3/P4 配额", QGuardRole.SUPER_ADMIN),
            ...
            _cmd("/授权 预检 [S3|P4] MAC", "预检设备能否在线激活", QGuardRole.SUPER_ADMIN),
